...
### 🧠 SmartModelRouter (New!)

OpenClaw now includes a **SmartModelRouter** that automatically selects the most cost-effective model based on your input complexity.

- **Automated Routing**: Analyzes prompt complexity (code, math, multi-step logic) to choose between `cheap`, `mid`, and `premium` tiers.
- **Cost Optimization**: Uses lighter models for simple tasks and powerful models only when necessary.
- **Smart Fallbacks**: Automatically falls back to available providers if API keys are missing.
- **Easy Configuration**: Enable it in `~/.openclaw/openclaw.yaml` with `smartRouting.enabled: true`.
